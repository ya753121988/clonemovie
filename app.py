...
# --- Flask Initial Setup ---
app = Flask(__name__)
app.secret_key = "STANDALONE_ULTIMATE_PREMIUM_FINAL_VERSION_100"

# --- Master MongoDB (নেটওয়ার্কের সব সাইটের লিস্ট এখানে থাকবে) ---
# এটি আপনার প্রধান ডাটাবেস। ক্লোন করলে ইউজার নতুন ডাটাবেস দিবে।
...
master_db = master_client['network_master_final']
sites_col = master_db['all_sites']

# --- ডাটাবেস কানেকশন ক্যাশ ---
db_cache = {}

def get_db(site_name):
    if site_name in db_cache: return db_cache[site_name]
...
# --- প্রফেশনাল রেসপন্সিভ লেআউট ইঞ্জিন (Ad Injection & Slider) ---
def render_master_ui(content, config):
    ...
    <head>
        ...
        <style>
            ...
            .copy-card {{ background: #111; padding: 20px; border-radius: 20px; border: 1px dashed #10b981; cursor: pointer; }}
        </style>
    ...
    <body>
            ...
            <div class="container mx-auto px-4 py-4 flex justify-between items-center text-white">
                <div class="flex items-center gap-4">
                    ...
                    <a href="/{sn}/" class="flex items-center gap-2">
                        <img src="{config['logo_url']}" class="h-8 md:h-10 rounded">
        ...
        <script>
            ...
            function copyTxt(id) {{ var t = document.getElementById(id).innerText; navigator.clipboard.writeText(t); alert("Copied to Clipboard! ✅"); }}
        </script>
...
def edit_movie(site_name, id):
    ...
    file_inputs = "".join([f'<div class="flex gap-2 mb-2"><input name="f_label" value="{f["quality"]}" class="bg-black/50 p-2 rounded-xl border border-white/10 w-1/3 text-white"><input name="f_ref" value="{f["file_ref"]}" class="bg-black/50 p-2 rounded-xl border border-white/10 w-2/3 text-white"></div>' for f in m['files']])
    content = f"""<div class="max-w-2xl mx-auto bg-slate-900 p-10 rounded-[40px] border border-white/5 shadow-2xl mt-4"><h2 class="text-2xl font-black mb-8 uppercase text-blue-500">Edit Complete Info 🎬</h2><form method="POST" class="space-y-4 font-black uppercase text-[11px] text-white"><input name="title" value="{m['title']}" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none text-white"><div class="grid grid-cols-2 gap-4"><input name="year" value="{m['year']}" class="bg-black/50 p-4 rounded-2xl border border-white/10 text-white"><input name="rating" value="{m['rating']}" class="bg-black/50 p-4 rounded-2xl border border-white/10 text-white"></div><input name="category" value="{m['category']}" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 text-white"><textarea name="plot" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 h-40 text-white">{m['plot']}</textarea><h3 class="text-emerald-500 underline uppercase tracking-widest mt-6 mb-2">Edit Download Buttons</h3>{file_inputs}<div class="text-blue-400 text-[10px] mb-2">Add New Button:</div><div class="flex gap-2"><input name="f_label" placeholder="Quality" class="bg-black/50 p-2 rounded-xl border border-white/10 w-1/3 text-white"><input name="f_ref" placeholder="File Ref ID" class="bg-black/50 p-2 rounded-xl border border-white/10 w-2/3 text-white"></div><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg mt-8">Save All Changes</button></form></div>"""
    return render_master_ui(content, config)
...
# --- Clone System: লিং জেনারেশন এবং কপি বাটন ---
...
def clone_page():
    ...
    if request.method == 'POST':
        ...
        if sites_col.find_one({"name": name}): flash("❌ Name Already Exists!")
            ...
            sites_col.insert_one({"name": name, "display_name": name + " Hub", "db_uri": db_uri, "logo_url": config_m['logo_url'], "header_notice": "Welcome!", "admin_user": "admin", "admin_pass": generate_password_hash("admin123"), "categories": ["Action", "Hindi"], "tmdb_key": config_m['tmdb_key'], "bot_token": "", "bot_username": "", "ads": {}})
            site_url = f"https://{request.host}/{name}/"
            admin_url = f"https://{request.host}/{name}/admin"
            content = f"""<div class="max-w-xl mx-auto bg-slate-900 p-10 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-6 text-white uppercase font-black"><h2 class="text-3xl text-emerald-400 mb-6 italic tracking-tighter">🎉 CLONE SUCCESSFUL! 🎉</h2><div class="space-y-4 text-left"><div class="copy-card"><p class="text-[9px] text-blue-400">YOUR SITE URL</p><p id="s_url" class="font-mono text-sm truncate">{site_url}</p><button onclick="copyText('s_url')" class="bg-blue-600 px-4 py-1 rounded-full text-[10px] mt-2 shadow-xl">COPY SITE URL</button></div><div class="copy-card"><p class="text-[9px] text-emerald-400">ADMIN PANEL URL</p><p id="a_url" class="font-mono text-sm truncate">{admin_url}</p><button onclick="copyText('a_url')" class="bg-emerald-600 px-4 py-1 rounded-full text-[10px] mt-2 shadow-xl">COPY ADMIN URL</button></div></div><p class="mt-6 text-[10px] opacity-60">Admin User: admin | Pass: admin123</p><a href="/" class="mt-8 inline-block bg-white text-black px-10 py-3 rounded-full text-xs font-bold shadow-2xl">BACK HOME</a></div>"""
            return render_master_ui(content, config_m)
    return render_master_ui("""<div class="max-w-md mx-auto bg-slate-900 p-12 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-10 text-white"><h2 class="text-3xl font-black mb-6 text-emerald-500 uppercase italic tracking-tighter">Clone Premium Network 🚀</h2><form method="POST" class="space-y-6"><input name="name" placeholder="Unique Site Name (Ex: Bkash)" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 font-bold text-white outline-none text-center shadow-inner"><input name="db_uri" placeholder="Paste Your MongoDB Atlas URI" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 text-[10px] text-white outline-none text-center shadow-inner" required><button class="w-full bg-emerald-600 hover:bg-emerald-700 py-5 rounded-3xl font-black uppercase tracking-widest text-white shadow-xl">Deploy My Network</button></form></div>""", config_m)

# --- Telegram Bot Webhook: All-in-One Engine ---
...
def webhook():
        ...
        if len(p) > 1:
            ...
            for s in all_s:
                ...
                if f:
                    ...
                    if s.get('auto_delete_min', 0) > 0:
                        threading.Timer(s['auto_delete_min']*60, lambda: bot.delete_message(m.chat.id, sent.message_id)).start()
                    return
    ...
    def handle_post(m):
        ...
        db_s, config_s = get_site_db(sn)
        if not db_s: return bot.reply_to(m, "❌ Site Not Found!")
        res = requests.get(f"https://api.themoviedb.org/3/search/movie?api_key={config_s['tmdb_key']}&query={mq}").json()
    ...
    def handle_sel(c):
        _, sn, tid = c.data.split('_'); db_s, config_s = get_site_db(sn)
