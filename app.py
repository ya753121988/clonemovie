...
app = Flask(__name__)
app.secret_key = "ultra_pro_cinema_v300_final"
...
<body>
    ...
    <!-- Actor Detail Modal -->
    <div id="actorModal" class="modal" onclick="this.style.display='none'">
        <div class="bg-zinc-900 border border-zinc-800 p-8 rounded-[2.5rem] max-w-2xl w-full max-h-[80vh] overflow-y-auto relative" onclick="event.stopPropagation()">
            <button class="absolute top-5 right-5 text-2xl" onclick="document.getElementById('actorModal').style.display='none'">&times;</button>
            <div id="actorContent" class="flex flex-col md:flex-row gap-8">
                <!-- Data will load here -->
            </div>
        </div>
    </div>
    ...
    <script>
        async function showActor(id) {{
            ...
            content.innerHTML = '<div class="w-full text-center py-10"><i class="fa fa-spinner fa-spin text-4xl text-rose-600"></i></div>';
            
            try {{
                const res = await fetch(`/get_person/${{id}}`);
                const p = await res.json();
                content.innerHTML = `
                    <img src="${{p.photo}}" class="w-48 h-64 object-cover rounded-3xl border border-zinc-700 mx-auto">
                    <div class="flex-1">
                        <h2 class="text-3xl font-black text-rose-600 mb-2">${{p.name}}</h2>
                        <p class="text-xs font-bold text-zinc-500 mb-4 uppercase tracking-widest">${{p.job}}</p>
                        <div class="space-y-3 text-sm text-zinc-300">
                            <p><strong>Born:</strong> ${{p.birthday || 'N/A'}}</p>
                            <p><strong>Place of Birth:</strong> ${{p.place || 'N/A'}}</p>
                            <p class="leading-relaxed text-zinc-400 mt-4">${{p.bio || 'No biography available.'}}</p>
                        </div>
                    </div>
                `;
            }} catch(e) {{
...
def home():
    ...
    '''
    return render_template_string(render_page("Explore", body), items=items)

# --- ডিটেইল পেজ (Backdrop & Clickable Cast Fixed) ---
...
def details(tid):
    ...
    <div class="relative min-h-screen pb-20">
        <!-- Backdrop Thumbnail (The large banner) -->
        ...
        <div class="container mx-auto px-4 -mt-80 relative z-10">
            <div class="flex flex-col lg:flex-row gap-12">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2 mb-8 uppercase text-[10px] font-bold">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 bg-zinc-900/50 p-6 rounded-[2rem] border border-zinc-800 text-center uppercase font-black text-[10px]">
                        <div><p class="text-zinc-500">Rating</p><p class="text-yellow-500 text-lg">⭐ {{ m.rating }}</p></div>
                        <div><p class="text-zinc-500">Language</p><p class="text-lg">{{ m.language }}</p></div>
                        <div><p class="text-zinc-500">Director</p><p class="text-rose-500 truncate">{{ m.director.name }}</p></div>
                        <div><p class="text-zinc-500">OTT</p><p class="text-green-500">{{ m.ott[0].name if m.ott else 'N/A' }}</p></div>
                    </div>

                    {% if m.ott %}
                    <div class="mb-10 p-6 bg-zinc-900/30 border border-zinc-800 rounded-[2.5rem]">
                        <p class="text-xs font-black uppercase text-rose-500 mb-6 tracking-widest">Streaming Platforms</p>
            ...
            <!-- Cast & Crew (Clickable for Details) -->
            <div class="mt-20"><h3 class="text-3xl font-black mb-10 border-l-4 border-rose-600 pl-4 uppercase italic">Cast & profiles (Click Photo)</h3>
                <div class="flex gap-10 overflow-x-auto pb-10">
                    <div class="min-w-[140px] text-center person-card cursor-pointer" onclick="showActor('{{ m.director.id }}')">
                        <img src="{{ m.director.photo }}" class="w-28 h-28 rounded-full mx-auto object-cover border-4 border-rose-600 shadow-2xl">
                    ...
                    <div class="min-w-[140px] text-center person-card cursor-pointer" onclick="showActor('{{ p.id }}')">
                        <img src="{{ p.photo }}" class="w-28 h-28 rounded-full mx-auto object-cover border-4 border-zinc-800">
                        <p class="mt-4 font-black text-sm">{{ p.name }}</p>
                        <p class="text-[9px] text-zinc-500 uppercase font-black">Producer</p>
                    </div>
                    {% endfor %}
                    {% for a in m.cast %}
                    <div class="min-w-[140px] text-center person-card cursor-pointer" onclick="showActor('{{ a.id }}')">
                        <img src="{{ a.photo }}" class="w-28 h-28 rounded-full mx-auto object-cover border-4 border-zinc-800">
            ...
            <!-- Gallery -->
            <div class="mt-20 pb-32"><h3 class="text-3xl font-black mb-10 border-l-4 border-rose-600 pl-4 uppercase italic">Gallery</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
...
def admin():
    ...
    return render_template_string(render_page("Admin", '<div class="h-[80vh] flex items-center justify-center px-4"><form method="POST" class="bg-zinc-900 p-12 rounded-[4rem] border border-zinc-800 w-full max-w-sm shadow-2xl"><h2 class="text-3xl font-black mb-10 text-center text-rose-600 uppercase italic">Admin Access</h2><input type="password" name="pw" placeholder="Password" class="w-full bg-black border border-zinc-700 p-5 rounded-3xl mb-6 text-center outline-none focus:border-rose-600 transition"><button class="w-full bg-rose-600 py-4 rounded-3xl font-black shadow-xl">LOG IN</button></form></div>'))
...
def dashboard():
    ...
    content = f'''
    <div class="container mx-auto py-10 px-4 max-w-5xl pb-20">
        <div class="flex justify-between items-center mb-12 border-b border-zinc-800 pb-8 uppercase italic">
            <h1 class="text-3xl font-black tracking-tighter italic">Control Hub</h1>
        ...
        <div class="grid md:grid-cols-2 gap-8 mb-16 text-center">
            <div class="bg-zinc-900 p-10 rounded-[3.5rem] border border-zinc-800 shadow-2xl">
                <h3 class="font-bold mb-6 text-blue-500 uppercase tracking-widest italic">Unlimited Auto Sync</h3>
            ...
            <div class="bg-zinc-900 p-10 rounded-[3.5rem] border border-zinc-800 shadow-2xl">
                <h3 class="font-bold mb-6 text-rose-500 uppercase tracking-widest italic">Single ID Sync</h3>
                <div class="flex flex-col gap-3 mb-6">
                    <input type="text" id="m_id" placeholder="TMDB ID" class="bg-black border border-zinc-700 p-4 rounded-xl outline-none text-white text-center">
                ...
                <button onclick="manualSync()" id="m_btn" class="w-full bg-rose-600 py-4 rounded-xl font-black uppercase">Sync This ID</button>
            </div>
        </div>

        <div class="bg-zinc-900 rounded-[3rem] border border-zinc-800 overflow-hidden">
            <table class="w-full text-left text-xs uppercase font-black"><thead class="bg-zinc-800 text-zinc-500"><tr><th class="p-8">Content</th><th class="p-8 text-center">Action</th></tr></thead>
            <tbody class="divide-y divide-zinc-800 font-black">
                {{% for i in items %}}
...
@app.route('/get_person/<id>')
def get_person(id):
    url = f"https://api.themoviedb.org/3/person/{id}?api_key={TMDB_API_KEY}"
    d = requests.get(url).json()
    return jsonify({
        "name": d.get('name'),
        "photo": IMG_BASE + d.get('profile_path') if d.get('profile_path') else "https://via.placeholder.com/150",
        "bio": d.get('biography'),
        "birthday": d.get('birthday'),
        "place": d.get('place_of_birth'),
        "job": d.get('known_for_department')
    })
