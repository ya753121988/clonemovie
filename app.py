...
# --- Flask Initial Setup ---
app = Flask(__name__)
app.secret_key = "ultimate_premium_standalone_final_v500"

# --- MongoDB Connection ---
# আপনার দেওয়া মূল MongoDB URI
MONGO_URI = "mongodb+srv://Demo270:Demo270@cluster0.ls1igsg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGO_URI)
db = client['standalone_ultimate_db']
settings_col = db['settings']
movies_col = db['movies']
files_col = db['stored_files']
bot_states = db['bot_states']

# --- Initialize Settings (বিন্দু পরিমাণ মিসিং ছাড়া) ---
def init_settings():
...
# --- Helper: ফাইল নাম থেকে শুধু কোয়ালিটি (480p, 720p...) বের করা ---
def extract_quality(filename):
    qualities = ['2160p', '1440p', '1080p', '720p', '480p', '360p', '4K', '8K', 'BluRay', 'WEB-DL', 'HDRip']
    for q in qualities:
        if q.lower() in filename.lower():
            return q.upper()
    return "Download"

# --- Premium Layout Engine (Auto Mobile/Desktop Responsive) ---
def render_pro(body, settings):
    ...
    <head>
        ...
        <style>
            ...
            .card:hover {{ transform: scale(1.05); border-color: #3b82f6; box-shadow: 0 0 30px rgba(59, 130, 246, 0.4); }}
            ...
            .copy-btn {{ background: #10b981; cursor: pointer; transition: 0.3s; }}
    ...
    <div class="flex flex-col lg:flex-row gap-12 mt-4">
        ...
        <div class="flex-1">
            ...
            <div class="mt-10 p-8 bg-slate-900/50 rounded-3xl border border-white/5 shadow-inner">
                <h3 class="text-blue-500 font-black text-xs uppercase mb-4 tracking-widest text-white">Full Movie Storyline 📖</h3>
...
def admin_login():
    ...
    return render_pro("""<div class="max-w-sm mx-auto bg-slate-900 p-10 rounded-[40px] border border-blue-500 shadow-2xl mt-10 text-center"><h2 class="text-2xl font-black mb-8 uppercase text-blue-500">Admin Login ⚙️</h2><form method="POST" class="space-y-4"><input name="user" placeholder="Username" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none focus:border-blue-500 text-white"><input name="pass" type="password" placeholder="Password" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none focus:border-blue-500 text-white"><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg hover:bg-blue-700 transition">LOGIN</button></form></div>""", settings)
...
# --- Clone Center: Final Guide & Generator ---
...
def clone_page():
    ...
    if request.method == 'POST':
        ...
        content = f"""<div class="max-w-xl mx-auto bg-slate-900 p-10 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-6 text-white"><h2 class="text-3xl font-black text-emerald-400 uppercase mb-6 italic">🎉 Success! New Site Ready 🎉</h2><div class="space-y-4 text-left"><div class="bg-black/50 p-4 rounded-2xl border border-blue-500"><p class="text-[10px] text-blue-400 font-black">SITE URL</p><p id="surl" class="font-mono text-sm truncate">{site_url}</p><button onclick="copyText('surl')" class="bg-blue-600 px-3 py-1 rounded mt-2 text-[10px] font-bold">COPY</button></div><div class="bg-black/50 p-4 rounded-2xl border border-emerald-500"><p class="text-[10px] text-emerald-400 font-black">ADMIN URL</p><p id="aurl" class="font-mono text-sm truncate">{admin_url}</p><button onclick="copyText('aurl')" class="bg-emerald-600 px-3 py-1 rounded mt-2 text-[10px] font-bold">COPY</button></div></div><p class="mt-6 text-xs opacity-60 italic">Now use this MongoDB URI in your GitHub deployment settings.</p></div>"""
        return render_pro(content, settings)
    content = f"""<div class="max-w-md mx-auto bg-slate-900 p-12 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-10"><h2 class="text-3xl font-black mb-6 text-emerald-500 uppercase tracking-tighter italic text-white">Network Cloner 🚀</h2><form method="POST" class="space-y-6"><input name="name" placeholder="Name Your Site" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 font-bold text-white outline-none text-center"><input name="db_uri" placeholder="Paste New MongoDB Atlas URI" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 text-xs text-white outline-none text-center" required><button class="w-full bg-emerald-600 hover:bg-emerald-700 py-5 rounded-3xl font-black uppercase tracking-widest text-white shadow-xl">Deploy My Instance</button></form></div>"""
    return render_pro(content, settings)

# --- Telegram Webhook Implementation (Premium File Server) ---
...
def webhook():
    ...
    def handle_start(m):
        ...
        if len(args) > 1:
            ...
            if file_data:
                bot.send_document(m.chat.id, file_data['file_id'], caption=f"🎬 Movie: *{file_data['file_name']}*\n\n✅ Enjoy Your Movie! ✅", parse_mode="Markdown")
            else: bot.send_message(m.chat.id, "❌ Error: File link expired or deleted.")
        else: bot.send_message(m.chat.id, f"🎬 Welcome! I am the official File Server for {settings['site_name']}.")
    ...
    def handle_cat(c):
        ...
        bot.send_message(c.message.chat.id, "✅ Send Movie Files now. I will extract quality automatically. Send /done when finished.")
    ...
    def handle_files(m):
        ...
        bot.reply_to(m, f"📥 {quality} File Recorded! More or /done?")
    ...
    def handle_done(m):
        ...
        bot.send_message(m.chat.id, "🚀 MOVIE PUBLISHED LIVE ON WEBSITE! ✅")
