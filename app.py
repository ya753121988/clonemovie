...
# --- অ্যাপ কনফিগারেশন ---
app = Flask(__name__)
app.secret_key = "ULTIMATE_FINAL_PREMIUM_NETWORK_V1000"

# --- Master MongoDB (নেটওয়ার্ক কন্ট্রোলার) ---
# আপনার দেওয়া মূল কানেকশন স্ট্রিং
...
# --- ডাটাবেস কানেকশন হ্যান্ডলার (প্রতিটি সাইটের জন্য আলাদা ডিবি) ---
def get_site_resources(site_name):
...
# --- প্রিমিয়াম ডুপ্লে ডিজাইন ইঞ্জিন ---
def render_site(content, config):
    ads = config.get('ads', {})
    sn = config['name']
    ...
    <head>
        ...
        <style>
            ...
            .card {{ background: #0f172a; border: 1px solid rgba(255,255,255,0.05); transition: 0.4s; position: relative; overflow: hidden; }}
        ...
        <nav class="glass sticky top-0 z-50">
            <div class="container mx-auto px-4 py-4 flex justify-between items-center text-white">
                <div class="flex items-center gap-4">
                    <a href="/clone-site" class="bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase shadow-lg transition">🚀 CLONE</a>
                    <a href="/{sn}/" class="flex items-center gap-2">
                        <img src="{config['logo_url']}" class="h-8 md:h-10 rounded shadow-lg">
                        <span class="text-xl md:text-2xl font-black tracking-tighter m-hide text-white">{config['display_name']}</span>
                    </a>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <div class="hidden lg:flex gap-4 text-[11px] font-bold uppercase tracking-widest">{cats}</div>
                    <a href="/{sn}/admin" class="bg-slate-800 hover:bg-blue-600 px-4 py-1.5 rounded-full text-[10px] font-bold transition border border-white/10 shadow-xl">ADMIN</a>
                </div>
            </div>
        </nav>
...
def site_home(site_name):
    db, config = get_site_resources(site_name)
    if not db: return "<h1>Site Not Found</h1>", 404
...
def movie_page(site_name, id):
    ...
    content = f"""<div class="flex flex-col lg:flex-row gap-12 mt-4"><div class="w-full lg:w-1/3 shrink-0"><img src="{m['poster']}" class="rounded-[40px] shadow-2xl w-full border border-white/10 sticky top-24"></div><div class="flex-1"><h1 class="text-4xl md:text-8xl font-black text-white leading-tight uppercase tracking-tighter">{m['title']}</h1><div class="flex flex-wrap gap-4 mt-8"><span class="bg-blue-600 px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest">{m['category']}</span><span class="bg-slate-800 px-6 py-2 rounded-xl text-xs font-black uppercase text-yellow-500 shadow-xl font-black">⭐ {m['rating']}</span><span class="bg-slate-800 px-6 py-2 rounded-xl text-xs font-black uppercase">{m['year']}</span></div><div class="mt-10 p-8 bg-slate-900/50 rounded-3xl border border-white/5 shadow-inner"><h3 class="text-blue-500 font-black text-xs uppercase mb-4 tracking-widest text-white">The Storyline 📖</h3><p class="text-slate-400 text-lg md:text-xl leading-relaxed italic font-medium">"{m['plot']}"</p></div><div class="mt-8 grid md:grid-cols-2 gap-6 text-[13px]"><div class="bg-black/40 p-5 rounded-2xl border border-white/5"><b class="text-blue-500 uppercase block mb-1 tracking-widest text-[9px]">🎬 Directed By</b> {m.get('director', 'Unknown')}</div><div class="bg-black/40 p-5 rounded-2xl border border-white/5"><b class="text-blue-500 uppercase block mb-1 tracking-widest text-[9px]">🌟 Starring Cast</b> {m.get('cast', 'Unknown')}</div></div><div class="mt-12 space-y-4"><h2 class="text-3xl font-black uppercase tracking-tighter mb-6 flex items-center gap-3">📥 Download Options:</h2>{links}</div></div></div>"""
    return render_site(content, config)
...
def admin_login(site_name):
    ...
    return render_site(f"""<div class="max-w-sm mx-auto bg-slate-900 p-10 rounded-[40px] border border-blue-500 shadow-2xl mt-10 text-center"><h2 class="text-2xl font-black mb-8 uppercase text-blue-500">{site_name} Admin HUB</h2><form method="POST" class="space-y-4"><input name="user" placeholder="Username" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none text-white font-bold"><input name="pass" type="password" placeholder="Password" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none text-white font-bold"><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg">Authenticate</button></form></div>""", config)
...
def dashboard(site_name):
    ...
    m_list = "".join([f'<div class="flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5 mb-3 shadow-lg"><div class="flex items-center gap-4"><img src="{m["poster"]}" class="h-10 w-8 rounded-lg object-cover"><span class="text-xs font-black truncate w-40 uppercase text-white font-bold">{m["title"]}</span></div><div class="flex gap-2"><a href="/{site_name}/edit-movie/{m["_id"]}" class="bg-blue-600 px-3 py-1 rounded text-[9px] font-black uppercase">Edit</a><a href="/{site_name}/delete/{m["_id"]}" class="bg-red-600 px-3 py-1 rounded text-[9px] font-black uppercase" onclick="return confirm(\'Delete?\')">Delete</a></div></div>' for m in movies])

    content = f"""<div class="grid lg:grid-cols-2 gap-10"><div class="bg-slate-900 p-8 rounded-[40px] border border-white/5 shadow-2xl"><h2 class="text-xl font-bold mb-6 uppercase text-blue-500">⚙️ Master Configuration</h2><form method="POST" class="space-y-4 text-[10px] font-black uppercase tracking-widest text-white"><input type="hidden" name="action" value="save_config"><div class="grid grid-cols-2 gap-4"><input name="display_name" value="{config['display_name']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"><input name="logo_url" value="{config['logo_url']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"></div><input name="header_notice" value="{config['header_notice']}" class="w-full bg-black/50 p-4 rounded-xl border border-white/10 outline-none"><div class="grid grid-cols-2 gap-4"><input name="tmdb_key" value="{config['tmdb_key']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"><input name="bot_token" value="{config['bot_token']}" placeholder="Bot Token" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"></div><div class="grid grid-cols-2 gap-4"><input name="bot_username" value="{config.get('bot_username','')}" placeholder="Bot Username" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"><input name="channel_id" value="{config.get('channel_id','')}" placeholder="Channel ID (-100...)" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none"></div><div class="grid grid-cols-2 gap-4 bg-blue-600/10 p-4 rounded-[30px] border border-blue-600/20"><div class="text-blue-400">Auto Delete (Min):<input name="auto_delete_min" type="number" value="{config.get('auto_delete_min', 0)}" class="bg-black/50 p-2 rounded-xl mt-1 w-full"></div><div>Forward Off:<br><input name="restrict_forward" type="checkbox" {"checked" if config.get("restrict_forward") else ""} class="mt-3 scale-125"></div></div><input name="categories" value="{','.join(config['categories'])}" class="w-full bg-black/50 p-4 rounded-xl border border-white/10 outline-none"><h3 class="text-emerald-500 underline mt-4 tracking-widest">💰 Ads Management</h3><div class="grid grid-cols-2 gap-4"><textarea name="banner_ad" placeholder="Banner" class="bg-black/50 p-3 rounded-xl h-20">{config.get('ads',{}).get('banner_ad','')}</textarea><textarea name="popunder" placeholder="Popunder" class="bg-black/50 p-3 rounded-xl h-20">{config.get('ads',{}).get('popunder','')}</textarea></div><div class="p-6 bg-blue-600/10 rounded-[30px] border border-blue-600/20 mt-4"><input name="admin_user" value="{config['admin_user']}" class="w-full bg-black/60 p-4 rounded-xl border border-white/10 mb-2"><input name="new_pass" type="password" placeholder="Set New Password" class="w-full bg-black/60 p-4 rounded-xl border border-white/10 outline-none"></div><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg">Save All Settings</button></form><form method="POST" class="mt-4"><input type="hidden" name="action" value="set_webhook"><button class="w-full bg-emerald-600 py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">🛠️ Auto Connect Webhook</button></form></div><div class="bg-slate-900 p-8 rounded-[40px] border border-white/5 shadow-2xl h-[1000px] overflow-y-auto"><h2 class="text-xl font-bold mb-6 uppercase text-emerald-500">🎬 Manage Movies</h2><form action="/{site_name}/dashboard" method="GET" class="mb-4 flex gap-2"><input name="dq" placeholder="Search movies..." class="w-full bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-xs text-white"><button class="bg-blue-600 px-4 rounded-xl font-black text-xs uppercase shadow-lg">Filter</button></form>{m_list}</div></div>"""
    return render_site(content, config)
...
def edit_movie(site_name, id):
    ...
    if request.method == 'POST':
        ...
        db['movies'].update_one({"_id": ObjectId(id)}, {"$set": upd}); flash("✅ Updated!")
        return redirect(f'/{site_name}/dashboard')
    ...
    content = f"""<div class="max-w-2xl mx-auto bg-slate-900 p-10 rounded-[40px] border border-white/5 shadow-2xl mt-4"><h2 class="text-2xl font-black mb-8 uppercase text-blue-500">Edit Complete Movie Info 🎬</h2><form method="POST" class="space-y-4 font-black uppercase text-[11px] text-white"><input name="title" value="{m['title']}" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none"><div class="grid grid-cols-2 gap-4"><input name="year" value="{m['year']}" class="bg-black/50 p-4 rounded-2xl border border-white/10 text-white"><input name="rating" value="{m['rating']}" class="bg-black/50 p-4 rounded-2xl border border-white/10 text-white"></div><input name="category" value="{m['category']}" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 text-white"><textarea name="plot" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 h-48 text-white">{m['plot']}</textarea><h3 class="text-emerald-500 underline uppercase tracking-widest mt-6 mb-2">Edit Download Buttons</h3>{f_in}<div class="flex gap-4"><input name="f_label" placeholder="Quality Label" class="bg-black/50 p-4 rounded-2xl border border-white/10 w-1/3 text-white"><input name="f_ref" placeholder="Reference ID" class="bg-black/50 p-4 rounded-2xl border border-white/10 w-2/3 text-white"></div><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg mt-8">Save Changes</button></form></div>"""
    return render_site(content, config)

@app.route('/<site_name>/delete/<id>')
def delete_movie(site_name, id):
    if not session.get(f'auth_{site_name}'): return redirect(f'/{site_name}/admin')
    db, _ = get_site_resources(site_name)
    db['movies'].delete_one({"_id": ObjectId(id)}); flash("🗑️ Deleted!")
    return redirect(f'/{site_name}/dashboard')

# --- Clone System: ইউআরএল কপি সহ স্বতন্ত্র নেটওয়ার্ক ---

@app.route('/clone-site', methods=['GET', 'POST'])
def clone_page():
    db_m, main_cfg = get_site_resources("Main")
    if request.method == 'POST':
        ...
        if sites_col.find_one({"name": name}): flash("❌ Global Name Taken!")
            ...
            content = f"""<div class="max-w-xl mx-auto bg-slate-900 p-10 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-6 text-white uppercase font-black"><h2 class="text-3xl text-emerald-400 mb-6 italic tracking-tighter">🎉 CLONE SUCCESSFUL! 🎉</h2><div class="space-y-4 text-left"><div class="copy-box"><p class="text-[9px] text-blue-400">USER SITE URL</p><p id="s_url" class="font-mono text-sm truncate">{s_u}</p><button onclick="copyTo('s_url')" class="bg-blue-600 px-4 py-1 rounded-full text-[10px] mt-2">COPY SITE URL</button></div><div class="copy-box"><p class="text-[9px] text-emerald-400">ADMIN PANEL URL</p><p id="a_url" class="font-mono text-sm truncate">{a_u}</p><button onclick="copyTo('a_url')" class="bg-emerald-600 px-4 py-1 rounded-full text-[10px] mt-2">COPY ADMIN URL</button></div></div><p class="mt-6 text-[10px] opacity-60">Admin User: admin | Pass: admin123</p><a href="/" class="mt-8 inline-block bg-white text-black px-10 py-3 rounded-full text-xs font-bold">BACK HOME</a></div>"""
            return render_site(content, main_cfg)
    return render_site("""<div class="max-w-md mx-auto bg-slate-900 p-12 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-10 text-white"><h2 class="text-3xl font-black mb-6 text-emerald-500 uppercase italic tracking-tighter">Clone Premium Network 🚀</h2><form method="POST" class="space-y-6"><input name="name" placeholder="Network Name (No Spaces)" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 font-bold text-white outline-none text-center shadow-inner" required><input name="db_uri" placeholder="Paste NEW MongoDB URI" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 text-[10px] text-white outline-none text-center shadow-inner" required><button class="w-full bg-emerald-600 hover:bg-emerald-700 py-5 rounded-3xl font-black uppercase tracking-widest text-white shadow-xl">Deploy Site</button></form></div>""", main_cfg)

# --- Telegram Bot Webhook Implementation ---

@app.route('/telegram-webhook', methods=['POST'])
def webhook():
    _, m_cfg = get_site_resources("Main")
    ...
    def handle_start(m):
        ...
        else: bot.send_message(m.chat.id, f"🎬 Welcome! official Bot for Movie Network.")

    @bot.message_handler(commands=['post'])
    def handle_post(m):
        q = m.text.split(' ', 2)
        if len(q) < 3: return bot.reply_to(m, "Use: `/post SiteName MovieQuery`")
        sn, mq = q[1], q[2]; db_s, c_s = get_site_resources(sn)
    ...
    def handle_sel(c):
        _, sn, tid = c.data.split('_'); db_s, c_s = get_site_resources(sn)
    ...
    def handle_cat(c):
        _, sn, cat = c.data.split('_'); db_s, _ = get_site_resources(sn)
    ...
    def handle_files(m):
        for s in list(sites_col.find({})):
            ...
            if st and st.get('step') == 'file':
                ...
                return bot.reply_to(m, f"📥 {qual} Stored!")
    ...
    def handle_done(m):
        for s in list(sites_col.find({})):
            ...
            if st:
                ...
                bot.send_message(m.chat.id, f"🚀 Movie Live on {st['sn']}! ✅")
