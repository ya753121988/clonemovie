...
# --- Flask Initial Setup ---
app = Flask(__name__)
app.secret_key = "STANDALONE_STANDALONE_MASTER_ULTIMATE_V99"

# --- MongoDB Connection ---
# গিটহাবে ডেপ্লয় করার সময় Environment Variable এ MONGO_URI সেট করবেন।
# অথবা সরাসরি নিচে আপনার মংগোডিবি লিঙ্কটি বসিয়ে দিন।
...
db = client['movie_premium_standalone_db']
    ...
    if settings_col.count_documents({}) == 0:
        settings_col.insert_one({
            ...
            "ads": {
                ...
                "header_ad": "", "footer_ad": "", "negative_banner": ""
            }
        })
init_settings()

# --- Helper: কোয়ালিটি ডিটেকশন (বটের জন্য) ---
...
# --- Master UI Engine (Responsive & Ad Injection) ---
def render_pro(body_html, settings):
    ...
    <head>
        ...
        <style>
            ...
            .ad-container {{ display: flex; justify-content: center; margin: 20px 0; }}
        </style>
    ...
    <body>
        ...
        <div class="container mx-auto px-4 py-8">
            <div class="ad-container">{ads.get('banner_ad', '')}</div>
            {body_html}
            <div class="ad-container">{ads.get('footer_ad', '')}</div>
        </div>
        ...
        for s in sliders:
            ...
            <div class="swiper-slide relative">
                ...
                <div class="absolute bottom-10 left-6 md:left-12 text-left">
                    ...
                    <a href="/movie/{s['_id']}" class="bg-blue-600 hover:bg-blue-700 px-8 py-3 rounded-full font-black text-xs md:text-sm inline-block shadow-2xl transition">⚡ VIEW DETAILS</a>
                </div>
            </div>"""
    ...
    <div class="flex flex-col lg:flex-row gap-12 mt-4">
        ...
        <div class="flex-1">
            ...
            <div class="mt-10 p-8 bg-slate-900/50 rounded-3xl border border-white/5 shadow-inner">
                <h3 class="text-blue-500 font-black text-xs uppercase mb-4 tracking-widest text-white">The Storyline 📖</h3>
...
# --- Routes: Admin Dashboard (Search, Edit, Delete, Ads) ---
    ...
    <div class="grid lg:grid-cols-2 gap-10">
        <div class="bg-slate-900 p-8 rounded-[40px] border border-white/5 shadow-2xl">
            ...
            <form method="POST" class="space-y-4 text-[10px] font-black uppercase tracking-widest">
                ...
                <div class="grid grid-cols-2 gap-4"><input name="site_name" value="{settings['site_name']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"><input name="logo_url" value="{settings['logo_url']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"></div>
                <input name="header_notice" value="{settings['header_notice']}" class="w-full bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white">
                <div class="grid grid-cols-2 gap-4"><input name="tmdb_key" value="{settings['tmdb_key']}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"><input name="bot_token" value="{settings['bot_token']}" placeholder="Bot Token" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"></div>
                <div class="grid grid-cols-2 gap-4"><input name="bot_username" value="{settings['bot_username']}" placeholder="Bot Username" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"><input name="categories" value="{','.join(settings['categories'])}" class="bg-black/50 p-4 rounded-xl border border-white/10 outline-none text-white"></div>
                <h3 class="text-emerald-500 underline mt-4">💰 Ad Management (Codes)</h3>
                <div class="grid grid-cols-2 gap-4">
                    <textarea name="banner_ad" placeholder="Banner Ad" class="bg-black/50 p-3 rounded-xl border border-white/10 h-20 text-white">{settings.get('ads',{}).get('banner_ad','')}</textarea>
                    <textarea name="popunder" placeholder="Popunder" class="bg-black/50 p-3 rounded-xl border border-white/10 h-20 text-white">{settings.get('ads',{}).get('popunder','')}</textarea>
                    <textarea name="header_ad" placeholder="Header Ad" class="bg-black/50 p-3 rounded-xl border border-white/10 h-20 text-white">{settings.get('ads',{}).get('header_ad','')}</textarea>
                    <textarea name="footer_ad" placeholder="Footer Ad" class="bg-black/50 p-3 rounded-xl border border-white/10 h-20 text-white">{settings.get('ads',{}).get('footer_ad','')}</textarea>
                </div>
                <div class="p-6 bg-blue-600/10 rounded-[30px] border border-blue-600/20 mt-4">
                    <input name="admin_user" value="{settings['admin_user']}" class="w-full bg-black/60 p-4 rounded-xl border border-white/10 mb-2 outline-none text-white">
                    <input name="new_pass" type="password" placeholder="Set New Password" class="w-full bg-black/60 p-4 rounded-xl border border-white/10 outline-none text-white">
                </div>
                <button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg">Save Master Configuration</button>
            </form>
            <form method="POST" class="mt-4"><input type="hidden" name="action" value="set_webhook"><button class="w-full bg-emerald-600 py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">🛠️ Auto Set Bot Webhook</button></form>
        </div>
        <div class="bg-slate-900 p-8 rounded-[40px] border border-white/5 shadow-2xl h-[950px] overflow-y-auto">
            ...
            <form action="/dashboard" method="GET" class="mb-4 flex gap-2"><input name="dq" placeholder="Search by title..." class="w-full bg-black/40 p-3 rounded-xl border border-white/10 outline-none text-xs text-white"><button class="bg-blue-600 px-4 rounded-xl font-black text-xs uppercase shadow-lg">Filter</button></form>
            {movies_list}
        </div>
    </div>"""
    return render_pro(content, settings)
...
def edit_movie(id):
    ...
    m = movies_col.find_one({"_id": ObjectId(id)})
    if request.method == 'POST':
        # বাটন বা ফাইল এডিট লজিক (নিখুঁতভাবে কোড করা)
        new_files = []
        labels = request.form.getlist('f_label')
        refs = request.form.getlist('f_ref')
...
def clone_page():
    ...
    if request.method == 'POST':
        ...
        return f'<div style="background:#000;color:#fff;padding:50px;text-align:center;font-family:sans-serif;height:100vh;"><h1>✅ Clone Done!</h1><p>Your Site: {site_url}</p><p>New MongoDB: {db_uri}</p><p>Deploy this code with the new URI to launch!</p><button onclick="window.history.back()">Back</button></div>'
    
    content = f"""<div class="max-w-md mx-auto bg-slate-900 p-12 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-10"><h2 class="text-3xl font-black mb-6 text-emerald-500 uppercase tracking-tighter italic text-white">Network Cloner 🚀</h2><form method="POST" class="space-y-6"><input name="name" placeholder="Desired Site Name" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 font-bold text-white outline-none text-center"><input name="db_uri" placeholder="Your MongoDB Atlas URI" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 text-xs text-white outline-none text-center" required><button class="w-full bg-emerald-600 hover:bg-emerald-700 py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl">Launch Independent Site</button></form></div>"""
    return render_pro(content, settings)

# --- Telegram Bot Webhook Implementation (File Store) ---
...
def webhook():
    ...
    def handle_start(m):
        ...
        if len(args) > 1:
            ...
            if file_data:
                bot.send_document(m.chat.id, file_data['file_id'], caption=f"🍿 মুভি: *{file_data['file_name']}*\n\n🔥 Enjoy your movie! 🔥", parse_mode="Markdown")
            else: bot.send_message(m.chat.id, "❌ Error: File Not Found!")
        else: bot.send_message(m.chat.id, f"🎬 Welcome! I am the official File Store for {settings['site_name']}.")
    ...
    def handle_cat(c):
        ...
        bot.send_message(c.message.chat.id, "✅ Category Set. Now Send Movie Files. Send /done when finished.")
