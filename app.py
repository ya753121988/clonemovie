...
app = Flask(__name__)
app.secret_key = "ultimate_premium_standalone_final_v900"

# --- MongoDB Connection ---
MONGO_URI = "mongodb+srv://Demo270:Demo270@cluster0.ls1igsg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGO_URI)
db = client['standalone_ultimate_final_db']
settings_col = db['settings']
movies_col = db['movies']
files_col = db['stored_files']
bot_states = db['bot_states']

# --- Initialize Settings (নতুন ফিচার সহ) ---
def init_settings():
    if settings_col.count_documents({}) == 0:
        settings_col.insert_one({
            "site_name": "Premium Movie Vault 🍿",
            ...
            "bot_token": "",
            "bot_username": "",
            "auto_delete_min": 0, # ০ মানে ডিলেশন অফ
            "restrict_forward": False, # ফরওয়ার্ড অন/অফ
            "ads": {
...
def render_pro(body, settings):
    ...
    <head>
        ...
        <style>
            ...
            .copy-box {{ background: rgba(0,0,0,0.5); padding: 15px; border-radius: 15px; border: 1px solid #10b981; margin-bottom: 10px; cursor: pointer; }}
        </style>
...
def home():
    ...
    for m in movies:
        grid_html += f"""<a href="/movie/{m['_id']}" class="card rounded-3xl overflow-hidden group"><div class="relative overflow-hidden"><img src="{m['poster']}" class="h-64 md:h-80 w-full object-cover group-hover:scale-110 transition duration-500"><div class="absolute top-2 right-2 bg-black/80 px-2 py-1 rounded-lg text-[9px] font-black uppercase">⭐ {m.get('rating', '0.0')}</div></div><div class="p-4 text-center"><h3 class="text-[11px] font-black truncate uppercase text-slate-100 group-hover:text-blue-500 transition">{m['title']}</h3><p class="text-[10px] text-slate-500 mt-1 font-bold uppercase">{m['year']} | {m['category']}</p></div></a>"""
    grid_html += '</div>'
...
def admin_login():
    ...
    return render_pro("""<div class="max-w-sm mx-auto bg-slate-900 p-10 rounded-[40px] border border-blue-500 shadow-2xl mt-10 text-center text-white"><h2 class="text-2xl font-black mb-8 uppercase text-blue-500">Admin Login ⚙️</h2><form method="POST" class="space-y-4"><input name="user" placeholder="Username" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none focus:border-blue-500"><input name="pass" type="password" placeholder="Password" class="w-full bg-black/50 p-4 rounded-2xl border border-white/10 outline-none focus:border-blue-500"><button class="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase shadow-lg hover:bg-blue-700 transition">LOGIN</button></form></div>""", settings)
...
def clone_page():
    ...
    if request.method == 'POST':
        ...
        content = f"""<div class="max-w-xl mx-auto bg-slate-900 p-10 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-6 text-white uppercase font-black"><h2 class="text-3xl text-emerald-400 mb-6 italic tracking-tighter">🎉 CLONE SUCCESSFUL! 🎉</h2><div class="space-y-4 text-left"><div class="copy-box"><p class="text-[9px] text-blue-400">USER SITE URL</p><p id="s_url" class="font-mono text-sm truncate">{site_url}</p><button onclick="copyToClipboard('s_url')" class="bg-blue-600 px-4 py-1 rounded-full text-[10px] mt-2">COPY URL</button></div><div class="copy-box"><p class="text-[9px] text-emerald-400">ADMIN PANEL URL</p><p id="a_url" class="font-mono text-sm truncate">{admin_url}</p><button onclick="copyToClipboard('a_url')" class="bg-emerald-600 px-4 py-1 rounded-full text-[10px] mt-2">COPY ADMIN</button></div></div><p class="mt-6 text-[10px] opacity-60">Use the new MongoDB URI in your deployment.</p><a href="/admin" class="mt-8 inline-block bg-white text-black px-10 py-3 rounded-full text-sm">BACK</a></div>"""
        return render_pro(content, settings)
    content = f"""<div class="max-w-md mx-auto bg-slate-900 p-12 rounded-[50px] border border-emerald-500 shadow-2xl text-center mt-10 text-white"><h2 class="text-3xl font-black mb-6 text-emerald-500 uppercase italic tracking-tighter">Clone Premium 🚀</h2><form method="POST" class="space-y-6"><input name="name" placeholder="New Site Name" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 font-bold text-white outline-none text-center shadow-inner"><input name="db_uri" placeholder="Paste New MongoDB Atlas URI" class="w-full bg-black/50 p-5 rounded-3xl border border-white/10 text-xs text-white outline-none text-center shadow-inner" required><button class="w-full bg-emerald-600 hover:bg-emerald-700 py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl transition-all">Launch Standalone Network</button></form></div>"""
    return render_pro(content, settings)

# --- Telegram Bot Webhook (অটো ডিলিট ও ফরওয়ার্ড কন্ট্রোল সহ) ---
...
def webhook():
    ...
    def handle_start(m):
        ...
        if len(args) > 1:
            ...
            if file_data:
                # ফরওয়ার্ড রেস্ট্রিকশন চেক
                protect = settings.get('restrict_forward', False)
                # ফাইল পাঠানো
                sent_msg = bot.send_document(m.chat.id, file_data['file_id'], 
                    caption=f"🎬 মুভি: *{file_data['file_name']}*\n\n✅ Enjoy Your Movie! ✅", 
                    parse_mode="Markdown", protect_content=protect)
                
                # অটো ডিলিট টাইমার
                del_min = settings.get('auto_delete_min', 0)
                if del_min > 0:
                    threading.Timer(del_min * 60, lambda: bot.delete_message(m.chat.id, sent_msg.message_id)).start()
                    bot.send_message(m.chat.id, f"⚠️ এই ফাইলটি {del_min} মিনিট পর অটোমেটিক ডিলিট হয়ে যাবে।")
    ...
    def handle_post(m):
        ...
        if not q: return bot.reply_to(m, "Format: `/post Avatar`")
    ...
    def handle_cat(c):
        ...
        bot.send_message(c.message.chat.id, "✅ Send Files now. Send /done when finished.")
    ...
    def handle_files(m):
        ...
        bot.reply_to(m, f"📥 {qual} Recorded! More or /done?")
